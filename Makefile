...
str		+= ft_strequ.c ft_strcpy.c ft_strccmp.c ft_strchr.c
str		:= $(addprefix $(strdir)/, $(str))

toadir	:= toa
toa 	:= ft_itoa.c ft_lltoa.c ft_stoa_base.c ft_ulltoa_base_caps.c
...
toa 	+= ft_ztoa.c ft_utoa_base.c ft_ztoa_base.c ft_ftoa.c
toa		:= $(addprefix $(toadir)/, $(toa))

utildir	:= utility
util	:= ft_atoi.c ft_atoi_base.c ft_isdigit.c
util	:= $(addprefix $(utildir)/, $(util))

objdir	:= obj
ddir	:= dep
src		:= $(ctrl) $(prt) $(toa) $(list) $(util) $(mem) $(str)
